...
extern int SCREEN_WIDTH, SCREEN_HEIGHT, MAZE_PADDING, WALL_THICKNESS,
    MAX_MAZE_SIZE, MIN_MAZE_SIZE, MIN_SCREEN_DIMENSION, MAX_SCREEN_DIMENSION,
    MAZE_WIDTH, CELL_LENGTH, x_0, y_0;
extern SDL_Renderer *renderer;
extern char *mazeDirectory;
extern SDL_Color BACKGROUND_COLOR, WALL_COLOR, GOAL_COLOR;
...
int loadMaze(Maze *maze, const char *path) {
  ...
  // Process two lines at a time up to second-to-last line and detect horizontal
  // and vertical walls
...
SDL_Texture *createMazeTexture(Maze *maze) {
  ...
  unsigned int bgColor = SDL_MapRGB(mazeSurface->format, BACKGROUND_COLOR.r,
                                    BACKGROUND_COLOR.g, BACKGROUND_COLOR.b);
  unsigned int wallColor =
      SDL_MapRGB(mazeSurface->format, WALL_COLOR.r, WALL_COLOR.g, WALL_COLOR.b);
  ...
  SDL_Rect mazeBorders[4] = {
      ...
      {x_0 + (CELL_LENGTH * maze->size), y_0, WALL_THICKNESS,
       (CELL_LENGTH * maze->size) + WALL_THICKNESS},
      {x_0, y_0 + (CELL_LENGTH * maze->size),
       (CELL_LENGTH * maze->size) + WALL_THICKNESS, WALL_THICKNESS}};

  // Render vertical walls
  int x = x_0 + CELL_LENGTH;
  int y = y_0;

  SDL_Rect goal = {(x_0 + (2 * WALL_THICKNESS)) +
                       (maze->finish_pos[0] * CELL_LENGTH),
                   y_0 + (2 * WALL_THICKNESS) +
                       ((maze->size - maze->finish_pos[1] - 1) * CELL_LENGTH),
                   CELL_LENGTH - 15, CELL_LENGTH - 15};
  Uint32 goal_color =
      SDL_MapRGB(mazeSurface->format, GOAL_COLOR.r, GOAL_COLOR.g, GOAL_COLOR.b);
  SDL_FillRect(mazeSurface, &goal, goal_color);
  ...
  for (int i = 0; i < maze->size; i++) {
    ...
    for (int j = 0; j < maze->size - 1; j++) {
      if (maze->vWalls[i][j]) {
        SDL_Rect tempRect = {x, y, WALL_THICKNESS,
                             CELL_LENGTH + WALL_THICKNESS};
  ...
  for (int i = 0; i < maze->size - 1; i++) {
    ...
    for (int j = 0; j < maze->size; j++) {
      if (maze->hWalls[i][j]) {
        SDL_Rect tempRect = {x, y, CELL_LENGTH + WALL_THICKNESS,
                             WALL_THICKNESS};
        SDL_FillRect(mazeSurface, &tempRect, wallColor);
      }
  ...
  SDL_Texture *mazeTexture =
      SDL_CreateTextureFromSurface(renderer, mazeSurface);
