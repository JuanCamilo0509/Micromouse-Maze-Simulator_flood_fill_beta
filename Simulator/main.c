...
int MAZE_PADDING =
    50; // Padding between maze border and edge of screen (pixels)
...
int MIN_SCREEN_DIMENSION; // Store the smaller screen dimension out of the width
                          // and height (pixels)
int MAX_SCREEN_DIMENSION; // Store the larger screen dimension out of the width
                          // and height (pixels)
char *mazePath =
    "default.txt"; // Name of default maze file used if no maze path provided
...
SDL_Color GOAL_COLOR = {228, 245, 39};
  ...
  while (1) {
    ...
    while (SDL_PollEvent(&e) != 0) {
      ...
      // Handle key presses
        ...
        // I: Credits :)
        case SDLK_i:
          printf("\nMaze Simulator v1.0\nCreated by Tyler Price for IEEE@UCLA "
                 "Micromouse\n\n");
          break;
...
void renderScreen() {
  ...
  // Render mouse if it is in motion, otherwise poll solver for next mouse
  // action
  if (!renderMouse(&mouse))
    getNextAction(&mouse);
...
int init() {
  ...
  window = SDL_CreateWindow(
      "Maze Simulator", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
